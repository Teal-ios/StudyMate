...
final class StudyRequestAPI {
    
    // 싱글톤 패턴
    static let shared: StudyRequestAPI = StudyRequestAPI()
    ...
    func requestStudy(userID: String, completionHandler: @escaping (studyRequestError?, Int?) -> Void) {
        MyPageProvider.request(.studyrequest(uid: userID)) { result  in
